...
int notify(char *notification_title, char* notification_body, char* notification_type) {
...
static void print_volume(   __attribute__((unused)) pa_context *c,
    ...
    Command *command = (Command *) userdata;
    char output[4] = "---";
    if (!i->mute) snprintf(output, 4, "%d", normalize(pa_cvolume_avg(&(i->volume))));
